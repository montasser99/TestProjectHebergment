...
export default function Edit({ auth, typeProduit }) {
    const { t } = useTranslation();
    const { data, setData, put, processing, errors } = useForm({
        name: typeProduit.name || '',
    });
    ...
    const submit = (e) => {
        ...
        put(route('admin.type-produits.update', typeProduit.id), {
            onSuccess: () => {
                instantToast.success(t('productTypeUpdatedSuccess', { name: data.name }));
            },
            onError: (errors) => {
                instantToast.error(t('errorUpdatingProductType'));
            }
        });
    };
            ...
            header={
                <div className="flex items-center space-x-4">
                    ...
                    <h2 className="text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200">
                        {t('editProductType')} : {typeProduit.name}
                    </h2>
                </div>
            }
        >
            <Head title={`${t('editProductType')} - ${typeProduit.name}`} />
