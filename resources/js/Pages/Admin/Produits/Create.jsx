...
export default function Create({ auth, typeProduits, priceMethodes }) {
    ...
    const handleImageChange = (e) => {
        ...
        if (file) {
            ...
            if (!allowedTypes.includes(file.type)) {
                instantToast.error('Type de fichier non autorisé. Utilisez JPEG, PNG, JPG, GIF ou WebP.');
                e.target.value = '';
                return;
            }
            
            if (file.size > maxSize) {
                instantToast.error('Le fichier est trop volumineux. Maximum 5MB autorisé.');
                e.target.value = '';
                return;
            }
    ...
    const submit = (e) => {
        ...
        post(route('admin.produits.store'), {
            forceFormData: true,
            onSuccess: () => {
                instantToast.success(t('productCreatedSuccess', { name: data.label }));
            },
            onError: (errors) => {
                console.log('Erreurs de validation:', errors);
                if (errors.image) {
                    instantToast.error(errors.image);
                } else if (errors.error) {
                    instantToast.error(errors.error);
                } else {
                    instantToast.error(t('errorCreatingProduct'));
                }
            }
        });
    };
                    ...
                    <form onSubmit={submit} className="space-y-6 sm:space-y-8">
                        ...
                        <div className="bg-white dark:bg-gray-800 overflow-hidden shadow-sm rounded-lg border border-gray-200 dark:border-gray-700">
                            <div className="p-4 sm:p-6">
                                ...
                                <div className="grid grid-cols-1 gap-4 sm:gap-6">
                                    {/* Nom du produit avec limite de 100 caractères */}
                        ...
                        <div className="flex items-center justify-end space-x-4">
                            ...
                            >
                                ...
                                <span>{processing ? t('creating') : t('save')}</span>
                            </button>
                        </div>
                    </form>
                </div>
